...
func schedulePod(pod apiobject.Pod) {
   ...
   if err := updatePodStatusAndNode(pod.Name, "nodeNameForExample", "Scheduled"); err != nil {
        log.Printf("Failed to update status for pod %s: %v", pod.Name, err)
        return
    }

    log.Printf("Pod %s scheduled to %s", pod.Name, nodeURL)
}
